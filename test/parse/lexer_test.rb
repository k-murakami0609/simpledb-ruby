...
class LexerTest < Minitest::Test
  ...
  def test_lexer
    lexer = Lexer.new("select test, test2 from test_table where A = 1 and B = 'test'")
    assert_equal lexer.eat_keyword_constant("Select"), "select"
    assert_equal lexer.eat_identifier, "test"
    assert_equal lexer.eat_delim(","), ","
    assert_equal lexer.eat_identifier, "test2"
    assert_equal lexer.eat_keyword_constant("from"), "from"
    assert_equal lexer.eat_identifier, "test_table"
    assert_equal lexer.eat_keyword_constant("where"), "where"
    assert_equal lexer.eat_identifier, "A"
    assert_equal lexer.eat_delim("="), "="
    assert_equal lexer.eat_int_constant, 1
    assert_equal lexer.eat_keyword_constant("and"), "and"
